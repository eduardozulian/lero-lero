<?php
/*
Plugin Name: Lero Lero Generator
Plugin URI: https://github.com/eduardozulian/lero-lero/
Description: The legendary Brazilian dummy text generator now on WordPress.
Version: 0.1
 ...
 * @return string $paragraph The dummy text 
 */
function lero_lero( $atts ) {
	...
	$num_paragraphs = intval( $p );
	$num_sentences = intval( $s );
	...
	while ( $num_paragraphs > 0 )
	{
		...
		for ( $i = 0; $i < $num_sentences; $i++ )
		{
			...
			$sentence .=  $tab0[$i] . $tab1[$i] . $tab2[$i] . $tab3[$i];
			$sentence .= '&nbsp;';
 ...
 * 
 */
function lero_lero_register_shortcodes() {

	add_shortcode( 'lero-lero', 'lero_lero' );
	add_shortcode( 'lerolero', 'lero_lero' );
...
add_action( 'init', 'lero_lero_register_shortcodes' );
